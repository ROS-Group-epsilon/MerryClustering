...
MerryMotionplanner::MerryMotionplanner(ros::NodeHandle* nodehandle):
		nh_(*nodehandle),
		cart_move_action_client_("cartMoveActionServer", true) { // constructor
    ROS_INFO("in constructor of MerryMotionplannerInterface");
...
//int g_return_code=0;
void MerryMotionplanner::doneCb_(const actionlib::SimpleClientGoalState& state,
...
void MerryMotionplanner::send_test_goal(void) {
    ROS_INFO("sending a test goal");
    cart_goal_.command_code = cwru_action::cwru_baxter_cart_moveGoal::ARM_TEST_MODE;
    cart_move_action_client_.sendGoal(cart_goal_, boost::bind(&MerryMotionplanner::doneCb_, this, _1, _2)); // we could also name additional callback functions here, if desired
...
int MerryMotionplanner::plan_move_to_pre_pose(void) {
...
int MerryMotionplanner::rt_arm_execute_planned_path(void) {
...
int MerryMotionplanner::rt_arm_request_q_data(void) {
...
Eigen::VectorXd MerryMotionplanner::get_right_arm_joint_angles(void) {
...
int MerryMotionplanner::rt_arm_request_tool_pose_wrt_torso(void) {
